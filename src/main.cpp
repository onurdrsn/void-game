...
// ── Entry Point ───────────────────────────────────────────────────────────────
  ...
  int main(int /*argc*/, char** /*argv*/) {
    ...
    fprintf(stderr, "[VOID] Starting application...\n");

    if(!platform_init("VOID", 1280, 720)) {
        fprintf(stderr, "[VOID] Failed to initialize platform\n");
        platform_fatal("Failed to initialize platform");
    }
    fprintf(stderr, "[VOID] Platform initialized\n");

    game_init();
    fprintf(stderr, "[VOID] Game initialized, state=%d\n", g_game.state);
